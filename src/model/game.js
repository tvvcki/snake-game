...
export function setDirection(state, x, y) {
  ...
  if (!sameDirection(state.nextDirection, next)) {
    ...
    state.lastEvent = 'turn';
  }
  state.nextDirection = next;
}

export function draw(ctx, state) {
  // draw is intentionally left for the view layer. Keep a placeholder so existing code can still call it.
  // The original canvas draw implementation lives in the old `js/game.js` file and will be ported to the Pixi view.
  console.warn('draw() called from model module — move rendering to view layer');
}
